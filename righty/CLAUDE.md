...
**各タスク完了後、必ず以下の検証フェーズを実行すること。**

### 検証フロー
1. **デプロイ完了後**、修正したページのURLにアクセス
2. **WebFetchツール**または**スクリーンショット確認**で画面を検証
3. **修正が正しく反映されているか**を確認
4. **問題がある場合**は原因を調査し再修正
...
### 検証コマンド例
```
WebFetch: 修正したURLの画面を確認
Read: ユーザーから提供されたスクリーンショットを確認
```

**注意:** 検証なしにタスクを完了としないこと。必ず実際の画面で動作確認を行う。
